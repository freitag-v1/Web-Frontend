...
export default {
    ...
    data() {
        return {
            ...
            currentPage: 1,
            perPage : 10,
            fields: [{key : 'projectDto.projectName', label: '프로젝트 이름'}, {key:'projectDto.workType', label: '작업 종류'},
    ...
    async created() {
        ...
        var requestProjectRes = await axios.get("/api/project/all");
        if(requestProjectRes.headers.list == 'none' || requestProjectRes.headers.login == 'fail'){
